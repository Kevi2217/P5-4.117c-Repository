...
homedata <- homedata %>%
  ...
  dplyr::select(-ejd_antaltoiletter, -pris_foersteudbud, -dato_aktueludbudpris, -ejd_ombygningsaar,
                ...
                -dato_annoncering, -kvhx,
                -adresse_postnr, -row_id, -salgstid, -hoejhus,
                -dwid_projektsalg, -beloeb_udbetaling, -beloeb_mdnetto, -beloeb_mdbrutto,
                -pris_aktueludbud, -antalstatusmoedersaelger, -afstemningsomraade, -pris_ejdvurdering) %>%
  ...
  dplyr::select(-ejdtype, -regionnavn, -postnr, -bynavn, -sogn, -corona, -periode_salg) %>%
  na.omit()
